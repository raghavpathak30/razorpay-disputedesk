...
| **[KEY]** Second pre-registration: clean-class re-grade (n=27), unsupported-assertion categorization | (a) 24/70 (34.3%), (b) 32/70 (45.7%), (c) 12/70 (17.1%), (d) 2/70 (2.9%) — bucket definitions in `DECISIONS.md`'s 2026-09-04 entry. This run: 38,412 tokens over 29 calls, 0/27 grader failures | `python -m eval.run_grounding_reregrade_clean27` — **not bit-reproducible**: live model non-determinism means a re-run grades the same 27 letters but will not return identical verdicts (confirmed: this run's own 24/27 flagged differs from the original run's 25/27 flagged on the identical 27 letters). The bucket categorization is a manual read on top of whatever the grader returns, disclosed in `DECISIONS.md`, and would need re-doing by hand against a fresh run's output, not merely re-run |
| **[KEY]** Second pre-registration: naturally-occurring contradictions in the "clean" class (descriptive, not pre-registered) | 13/27 letters (48.1%) contained a `"contradicted"` assertion; 13/18 (72.2%) of letters where `delivery_confirmed=False` asserted delivery/fulfillment anyway | same command as above; per-item quotes in `DECISIONS.md`'s 2026-09-04 entry |
